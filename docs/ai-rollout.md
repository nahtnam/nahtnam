...
4. Verify receipt/item deep links survive admin login; scanning is read-only. Verify Done/Snooze/Ignore, pending yes/no outcomes, Undo, old receipt state and source reconciliation before importing real reminders.
5. Configure `TWILIO_ACCOUNT_SID` and `TWILIO_AUTH_TOKEN` for the existing 1-855-NAHTNAM webhook and outbound exceptions. Check current toll-free verification and test an authorized exchange. Confirm Twilio signature checks, sender allowlist, version matching and duplicate handling. Commands must never print themselves. Verify delivery callbacks, distinguishing accepted from delivered. Never send routine summaries over SMS.
