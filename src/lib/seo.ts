...
export function canonicalUrl(path: string) {
  return new URL(path, appUrl).toString().replace(/\/$/, "") || appUrl;
}

export function ogImageUrl(options: OgImageOptions) {
  const imageUrl = new URL("/og", appUrl);
  imageUrl.searchParams.set("title", options.title);
  imageUrl.searchParams.set("description", options.description);
  imageUrl.searchParams.set("path", options.path);

  if (options.label) {
    imageUrl.searchParams.set("label", options.label);
  }
...
export function createSeo(options: SeoOptions) {
  ...
  const publishedAt =
    options.publishedAt instanceof Date
      ...
      : typeof options.publishedAt === "number"
