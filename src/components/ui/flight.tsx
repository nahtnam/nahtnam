...
function FlightAirport({
  ...
  const isOnScreen = useCoordinateVisibility(map, markerCoordinate);
  ...
  if (!isVisible || !isOnScreen) return null;
...
function interpolatePosition(
  ...
  if (t <= 0) return firstCoord;
  if (t >= 1) return lastCoord;
  ...
  if (i >= totalSegments) return lastCoord;
...
function isCoordinateVisibleOnMap(
  map: MapLibreGL.Map | null,
  ...
  const normalizedCoordinate = normalizeLngLat(coordinate);

  if (map.getProjection()?.type === "globe") {
    const center = map.getCenter();
    const centerLat = toRadians(center.lat);
    const pointLat = toRadians(normalizedCoordinate[1]);
    const deltaLng = toRadians(normalizedCoordinate[0] - center.lng);
    const cosine =
      Math.sin(centerLat) * Math.sin(pointLat) +
      Math.cos(centerLat) * Math.cos(pointLat) * Math.cos(deltaLng);
  ...
  try {
    const point = map.project(normalizedCoordinate);
...
function useCoordinateVisibility(
  map: MapLibreGL.Map | null,
  coordinate: [number, number],
): boolean {
  const [isVisible, setIsVisible] = useState(() =>
    isCoordinateVisibleOnMap(map, coordinate),
  );
  ...
  useEffect(() => {
    ...
    const updateVisibility = () => {
      setIsVisible(isCoordinateVisibleOnMap(map, coordinate));
    };

    updateVisibility();
    map.on("move", updateVisibility);
    map.on("resize", updateVisibility);
    map.on("projectiontransition", updateVisibility);

    return () => {
      map.off("move", updateVisibility);
      map.off("resize", updateVisibility);
      map.off("projectiontransition", updateVisibility);
    };
  }, [coordinate, map]);
...
function FlightAnimationMarker({
  ...
  const updateMarker = useCallback(
    (t: number) => {
      ...
      if (markerElRef.current) {
        markerElRef.current.style.visibility = isCoordinateVisibleOnMap(
          map,
          pos,
        )
          ? "visible"
  ...
  useEffect(() => {
    ...
    const animate = (timestamp: number) => {
      ...
      let t = effectiveElapsed / duration;

      if (t >= 1) {
        onCompleteRef.current?.();
        if (loop) {
          const remainder = effectiveElapsed % duration;
          startTimeRef.current = timestamp - remainder - delay;
  ...
  }, [isControlled, delay, duration, loop, arcCoords, updateMarker]);
...
function FlightMultiLegAnimationMarker({
  ...
  const computePositionAndBearing = useCallback(
    (
      ...
      for (let i = 0; i < legs.length; i++) {
        ...
        if (breakpoint !== undefined && t <= breakpoint) {
          legIndex = i;
          break;
        }
      ...
      const legStart = breakpoints[legIndex] ?? 0;
      const legEnd = breakpoints[legIndex + 1] ?? 1;
  ...
  }, [isControlled, delay, duration, loop, legData, updateMarker]);
