...
import { generateOgImagePng } from "../src/lib/og-image";
...
const outputDir = ".tmp/og-previews";
await mkdir(outputDir, { recursive: true });

await Promise.all(
  previews.map(async (preview) => {
    const png = await generateOgImagePng(preview);
    await writeFile(`${outputDir}/${preview.name}.png`, png);
  }),
);

console.log(`Wrote ${previews.length} OG image previews to ${outputDir}`);
