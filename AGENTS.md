...
- **Convex auth startup**: Keep the Vertex split between SSR auth and browser websocket auth. Route loaders may set `serverHttpClient` auth for SSR, but `ConvexProviderWithAuth` should own browser `convexClient.setAuth`/`clearAuth`; the shared client can still use `expectAuth: true`.
