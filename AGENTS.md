...
- **Route query preloading**: Any route component that calls a Convex route query hook (`useSuspenseQuery` or guarded `useQuery`) should preload the same helper in that route's loader/beforeLoad with matching args. If the data is private, gate the preload with the same auth condition that controls rendering so refreshes stay warm without leaking data.
- **Convex auth startup**: Keep the Vertex split between SSR auth and browser websocket auth. Route loaders may set `serverHttpClient` auth for SSR, but browser auth should be handled after AuthKit mounts; with `expectAuth: true`, logged-out clients need an explicit `convexClient.setAuth(async () => null)` once AuthKit finishes so the paused websocket resumes for public Convex route queries.
