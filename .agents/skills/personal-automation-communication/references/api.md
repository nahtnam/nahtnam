...
For coordinator coverage and delivery readback, run the client `get` **without `--source`**. This also returns `sources` (up to 50 source health reports, with `checkedAt`, `coverageThrough`, `status` and `stale`), `receipts` (20 most recently created receipts, with `idempotencyKey`, `printJobId`, `printStatus`, `printError` and `expired`), and `deliveries` (20 most recently updated SMS attempts, with id, item/version, milestone, idempotency key, provider id, status and update time; no phone number). Match receipts/attempts by their saved id or stable idempotency key. `operationsTruncated` separately flags incomplete `sources`, `receipts` and `deliveries` lists. Source-filtered snapshots omit these operational lists. Missing or truncated records are not proof of success, failure, or absence; keep uncertain sends unresolved and report the coverage gap rather than inventing a new send key. `stale` means no source check for over 24 hours; the coordinator must also apply each source's expected cadence. Readback reports stored worker/provider status; it does not actively query the SMS provider or prove physical paper output.
